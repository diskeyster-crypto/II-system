    ...
    if ($action === 'message') {
        ...
        if (!$userMsg) {
            ...
            try {
                ...
                // Check if reply contains final spec JSON marker
                if (str_contains($reply, '===FINAL_TASK_SPEC===')) {
                    $specPart = extractFinalSpec($reply);
                    if ($specPart) {
                        saveSpec($db, $taskId, $task, $specPart, $reply);
                ...
                $error = t('ai.request_failed') . ': ' . $e->getMessage();
...
5. Propose a risk level (low/medium/high/critical).
...
/** Extract structured fields from spec text and persist them on the task. */
function saveSpec(\PDO $db, int $taskId, array $task, string $specText, string $fullReply): void
{
    // Try to extract acceptance criteria
    $criteria = [];
    if (preg_match('/## Acceptance Criteria([\s\S]*?)(?:##|$)/i', $specText, $m)) {
        preg_match_all('/- \[[ x]\] (.+)/i', $m[1], $cm);
        $criteria = $cm[1] ?? [];
    }

    // Extract risk level
    $riskLevel = $task['risk_level'] ?? 'medium';
    if (preg_match('/## Risk Level\s*(low|medium|high|critical)/i', $specText, $m)) {
        $riskLevel = strtolower(trim($m[1]));
    }

    // Extract roadmap branch / item code
    $roadmapBranch   = '';
    $roadmapItemCode = '';
    if (preg_match('/## Roadmap Branch\s*(.+)/i', $specText, $m)) {
        $roadmapBranch = trim($m[1]);
    }
    if (preg_match('/## Roadmap Item Code\s*(.+)/i', $specText, $m)) {
        $roadmapItemCode = trim($m[1]);
    }

    // Extract allowed / forbidden / expected files
    $allowedFiles   = extractFileList($specText, 'Allowed Files');
    $forbiddenFiles = extractFileList($specText, 'Forbidden Files');
    $expectedFiles  = extractFileList($specText, 'Expected Files');

    $db->prepare(
        'UPDATE dev_tasks SET
            final_task_spec = ?, status = "ready_to_run",
...
function extractFileList(string $text, string $section): array
{
    if (!preg_match('/## ' . preg_quote($section, '/') . '([\s\S]*?)(?:##|$)/i', $text, $m)) {
        return [];
    }
    $block = $m[1];
    preg_match_all('/[-*] `?([^\s`\n]+)`?/i', $block, $cm);
    return array_values(array_filter($cm[1] ?? []));
}

$pageTitle = t('tasks.chat') . ': ' . $task['title'];
$activeNav = 'tasks';
...
<div class="flex gap-3" style="align-items:flex-start">
  ...
  <div style="flex:1.4">
    <div class="card">
      <div class="card-title">💬 <?= e(t('tasks.chat')) ?></div>

      <?php if (empty($messages)): ?>
        <p class="text-muted text-sm"><?= e(t('tasks.no_messages')) ?></p>
      <?php else: ?>
          ...
          <?php foreach ($messages as $msg): ?>
            <div>
              ...
              <div class="chat-meta <?= $msg['role'] === 'operator' ? 'text-right' : '' ?>">
                <?= $msg['role'] === 'operator' ? '👤 ' . e(t('tasks.you')) : '🤖 GPT' ?> · <?= htmlspecialchars(substr($msg['created_at'], 0, 16), ENT_QUOTES, 'UTF-8') ?>
              </div>
            </div>
          <?php endforeach; ?>
        </div>
      ...
      <form method="post" class="mt-3">
        ...
        <div class="form-group">
          <textarea name="message" id="msg-input" style="min-height:80px"
                    placeholder="<?= e(t('tasks.type_message')) ?>"><?= htmlspecialchars($_POST['message'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
        </div>
        <div class="flex gap-2">
          <button type="submit" class="btn btn-primary"><?= e(t('tasks.btn_send')) ?></button>
          <button type="submit" name="action" value="generate_spec" class="btn btn-secondary">⚡ <?= e(t('tasks.btn_spec')) ?></button>
        </div>
      </form>
    </div>
  </div>
  ...
  <div style="flex:1">
      ...
      <?php if ($task['final_task_spec']): ?>
        ...
        <?php if (in_array($task['status'], ['ready_to_run', 'clarifying'])): ?>
          ...
          <a href="<?= BASE_URL ?>/admin/tasks/view.php?id=<?= $taskId ?>#approve"
            ...
            ✅ <?= e(t('tasks.btn_approve')) ?> →
          </a>
        <?php endif; ?>
      <?php else: ?>
        <p class="text-muted text-sm"><?= e(t('tasks.spec_not_set')) ?></p>
      <?php endif; ?>
    </div>
    ...
    <div class="card">
      ...
      <table>
        <tr><td class="text-muted" style="width:90px"><?= e(t('common.status')) ?></td><td><span class="badge badge-draft"><?= e(t('statuses.' . $task['status'])) ?></span></td></tr>
        <tr><td class="text-muted"><?= e(t('tasks.priority')) ?></td><td><?= e(t('statuses.priority_' . $task['priority'])) ?></td></tr>
        <tr><td class="text-muted"><?= e(t('tasks.risk_level')) ?></td><td><?= e(t('statuses.risk_' . $task['risk_level'])) ?></td></tr>
